...
const RequestApiKey: FC<RequestApiKeyProps> = ({}) => {
  ...
  const [isApiKeyRequested, setIsApiKeyRequested] = useState<boolean>(false)
  ...
  async function createNewApiKey(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setIsApiKeyRequested(true) // set the state to true when the key is generated
    } catch (err) {
  ...
  return (
    <div className='container md:max-w-2xl'>
      <div className='flex flex-col gap-6 items-center'>
        ...
        {/* Render different paragraph based on whether the key is requested or not */}
        <Paragraph>{isApiKeyRequested ? 'Your API key.' : "You haven't requested an API key yet."}</Paragraph>
      </div>
      <form
